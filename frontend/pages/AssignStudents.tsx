...
const AssignStudents: React.FC = () => {
  ...
  const loadData = async () => {
    ...
    const deptStaff = allUsers.filter(u =>
      ...
      (user?.role === UserRole.ADMIN || deptBase === '' || !u.department || u.department?.startsWith(deptBase)) &&
    ...
    await Promise.all(deptStudents.map(async (s) => {
  ...
  const handleSave = async () => {
    ...
    const faculty = staff.find(s => String(s.id) === String(selectedStaffId));
    if (!faculty) {
      alert("Could not find selected faculty member. Please try again.");
      return;
    }

    try {
      await ApiService.assignStudentsToStaff(
        String(faculty.id),
        faculty.name,
        '', '',
        selectedStudentIds.map(String)
      );

      setSaveStatus(`✅ Registry Synchronized: ${selectedStudentIds.length} student(s) mapped to ${faculty.name}.`);
      setSelectedStudentIds([]);

      // Reload to show updated mentor names
      await loadData();
        ...
        {activeTab === 'LIST' ? (
          <div className="animate-in fade-in slide-in-from-bottom-2 duration-300 space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {staff.map(member => {
                const menteeCount = students.filter(s => String(s.mentorId) === String(member.id)).length;
                return (
                  <div
                    key={member.id}
                ...
                <div className="max-w-2xl">
                  <div className="space-y-3">
                    ...
                    <div className="relative">
                      ...
                      >
                        <option value="" disabled className="bg-surface-elevated text-text-muted">Choose Mentor...</option>
                        {staff.map(s => <option key={s.id} value={s.id} className="bg-surface-elevated text-text-primary font-bold">{s.name} ({s.designation || 'Faculty'} - {s.role})</option>)}
                      </select>
                      <svg className="absolute right-6 top-1/2 -translate-y-1/2 w-5 h-5 text-slate-500 pointer-events-none" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              ...
              <div className="p-4 space-y-3">
                {filteredStudents.map(s => {
                  ...
                  return (
                    <div
                      key={s.id}
                      onClick={() => handleToggleStudent(String(s.id))}
                      className={`w-full px-6 py-4 rounded-2xl border-2 transition-all cursor-pointer group flex flex-row items-center gap-6 shadow-sm hover:shadow-md ${selectedStudentIds.includes(String(s.id)) ? 'bg-primary/5 border-primary/40 shadow-primary/5' : 'bg-surface-elevated/40 border-border-subtle hover:border-primary/20'}`}
                    >
