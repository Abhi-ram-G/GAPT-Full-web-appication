...
import { PortalConnection, PortalConnectionStatus, PortalPermission } from '../types';
...
const PortalConnectionPage: React.FC = () => {
  ...
  useEffect(() => {
    const fetchPortals = async () => {
      const p = await ApiService.getPortals();
      setPortals(p);
    };
    fetchPortals();
  }, []);
  ...
  const handleAddPortal = async (e: React.FormEvent) => {
    ...
    await ApiService.addPortal(portalToAdd);
    const p = await ApiService.getPortals();
    setPortals(p);
  ...
  const handleConnect = async (id: string) => {
    await ApiService.updatePortal(id, { status: PortalConnectionStatus.PENDING });
    const p = await ApiService.getPortals();
    setPortals(p);
  ...
  const simulateRemoteApproval = async (id: string, stage: 'CONNECT' | 'MODIFY') => {
    if (stage === 'CONNECT') {
      await ApiService.updatePortal(id, { status: PortalConnectionStatus.CONNECTED, lastSync: new Date().toISOString() });
    } else {
      await ApiService.updatePortal(id, { permission: PortalPermission.READ_WRITE });
    }
    const p = await ApiService.getPortals();
    setPortals(p);
  ...
  const handleDelete = async (id: string) => {
    await ApiService.deletePortal(id);
    const p = await ApiService.getPortals();
    setPortals(p);
  };
  ...
  return (
    <DashboardLayout title="Institutional Interlink Control">
      <div className="max-w-6xl mx-auto py-2 space-y-8">
        ...
        {statusMsg && (
          <div className={`p-4 rounded-2xl border text-xs font-bold uppercase tracking-wider animate-in fade-in slide-in-from-top-4 duration-300 flex items-center gap-3 ${statusMsg.type === 'success' ? 'bg-emerald-500/10 border-emerald-500/20 text-emerald-400' :
          ...
          {portals.map(p => (
            <div key={p.id} className="bg-slate-900 border border-slate-800 rounded-[2rem] p-6 shadow-xl relative group overflow-hidden">
              ...
              <div className="absolute top-6 right-6 flex items-center space-x-2">
                <span className={`w-2 h-2 rounded-full ${p.status === PortalConnectionStatus.CONNECTED ? 'bg-emerald-500 shadow-[0_0_10px_rgba(16,185,129,0.5)]' :
                  ...
                  <div className="flex flex-col space-y-2">
                    <div className="flex gap-2">
                      <button
                        ...
                        className={`flex-1 py-3 rounded-xl text-[10px] font-bold uppercase tracking-[0.1em] transition-all ${p.permission === PortalPermission.READ_WRITE
