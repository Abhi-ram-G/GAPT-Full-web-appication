...
const SemesterDetail: React.FC = () => {
  ...
  // Persistence States
  const [editingLesson, setEditingLesson] = useState<{ subjectId: string, index: number, name: string } | null>(null);
  const [pendingFiles, setPendingFiles] = useState<Record<string, Record<number, string>>>({});
  ...
  const loadData = async () => {
    ...
    if (batchId && deptId) {
      const s = await ApiService.getCurriculumStatus(batchId, deptId);
      setStatus(s);
    }
  };
  ...
  const toggleStatus = async () => {
    ...
    await ApiService.setCurriculumStatus(batchId, deptId, nextStatus);
    await loadData();
  };
  ...
  const handleCreateSubject = async (addNewAfter = false) => {
    ...
    await ApiService.persistStructure(updatedCourses, batches);
    await loadData();
  ...
  const handleAddQP = async () => {
      ...
      if (c.id === deptId) {
        return {
          ...
          subjects: c.subjects.map(s => {
            if (s.id === newQP.subjectId) {
              const papers = [...(s.questionPapers || [])];
              const paper: QuestionPaper = {
                name: newQP.name.toUpperCase(),
                file: newQP.file || 'GAPT_AUTH_REV_NULL'
              };
              if (newQP.isEditing && newQP.editIndex !== -1) {
                papers[newQP.editIndex] = paper;
              } else {
                papers.push(paper);
              }
              return { ...s, questionPapers: papers };
            }
            return s;
          })
        };
    ...
    await ApiService.persistStructure(updatedCourses, batches);
    await loadData();
    setNewQP({ subjectId: semesterSubjects[0]?.id || '', name: '', file: '', isEditing: false, editIndex: -1 });
    setIsQPModalOpen(false);
    setSyncingStatus(newQP.isEditing ? "Paper Archive Updated." : "Assessment Resource Synchronized.");
    setTimeout(() => setSyncingStatus(null), 3000);
  };

  const handleUpdateLessonName = async () => {
    ...
    const updatedCourses = courses.map(c => {
      if (c.id === deptId) {
        return {
          ...c,
          subjects: c.subjects.map(s => {
            if (s.id === editingLesson.subjectId) {
              const names = [...(s.lessonNames || [])];
              while (names.length < s.lessonsCount) names.push('');
              names[editingLesson.index] = editingLesson.name.toUpperCase();
              return { ...s, lessonNames: names };
            }
            return s;
          })
        };
    ...
    setSyncingStatus("Module Nomenclature Synchronized.");
    setTimeout(() => setSyncingStatus(null), 2000);
  };

  const handleSaveMaterial = async (subjectId: string, index: number) => {
    const fileName = pendingFiles[subjectId]?.[index];
    if (!fileName || !activeCourse || !activeBatch) return;

    const subjectObj = semesterSubjects.find(s => s.id === subjectId);
    ...
    // Notification Protocol: Dispatch alerts to relevant students
    const allUsers = await ApiService.getUsers();
    const courseFullName = `${activeCourse.name} (${activeCourse.degree})`;
    const batchYearShort = activeBatch.startYear.toString().slice(-2);

    const targetStudents = allUsers.filter(u =>
      u.role === UserRole.STUDENT &&
      u.department === courseFullName &&
      u.regNo?.includes(`BIT${batchYearShort}`)
    );

    const moduleName = subjectObj.lessonNames?.[index] || `Module ${index + 1}`;

    for (const std of targetStudents) {
      await ApiService.addNotification({
        id: crypto.randomUUID(),
        userId: std.id,
        message: `Academic Release: New learning resource "${fileName}" deployed for ${subjectObj.name} (${moduleName}).`,
    ...
    setPendingFiles(prev => {
      const copy = { ...prev };
      if (copy[subjectId]) {
        delete copy[subjectId][index];
        if (Object.keys(copy[subjectId]).length === 0) delete copy[subjectId];
      }
      return copy;
    });

    setSyncingStatus("Resource Persistent. Alerts Dispatched.");
    setTimeout(() => setSyncingStatus(null), 3000);
  };
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (activeUpload.type === 'LESSON' && activeUpload.index !== undefined) {
      if (activeUpload.subjectId) {
        // Queue for "Save" action as per user request
        setPendingFiles(prev => ({
          ...prev,
          [activeUpload.subjectId!]: {
            ...(prev[activeUpload.subjectId!] || {}),
            [activeUpload.index!]: file.name
          }
        }));
        setSyncingStatus(`Queued: ${file.name}`);
      } else {
  ...
  const handleOpenAsset = (filename: string) => {
    if (!filename) return;
    alert(`Institutional Protocol: Opening secure asset archive for "${filename}"`);
  };

  const openQPModalWithSubject = (subjectId: string) => {
    setNewQP({ subjectId, name: '', file: '', isEditing: false, editIndex: -1 });
    setIsQPModalOpen(true);
  };

  const handleEditQP = (subjectId: string, paper: QuestionPaper, index: number) => {
    setNewQP({
      subjectId,
      name: paper.name,
      file: paper.file,
  ...
  const handleDeleteQP = async (subjectId: string, index: number) => {
    if (!confirm("Permanently delete this assessment record?")) return;
    ...
    const updatedCourses = courses.map(c => {
      if (c.id === deptId) {
        return {
          ...
          subjects: c.subjects.map(s => {
            if (s.id === subjectId) {
              const papers = (s.questionPapers || []).filter((_, i) => i !== index);
              return { ...s, questionPapers: papers };
            }
            return s;
          })
    ...
    await ApiService.persistStructure(updatedCourses, batches);
    await loadData();
    setSyncingStatus("Record Purged.");
    setTimeout(() => setSyncingStatus(null), 2000);
  };

  if (!activeBatch || !activeCourse) {
    return <DashboardLayout title="Registry Mismatch"><div className="py-32 text-center text-text-muted font-bold">Academic parameters unrecognized.</div></DashboardLayout>;
  }
  ...
  return (
    <DashboardLayout title={`${activeCourse.name} - Sem ${semNum}`}>
              ...
              <div className="flex flex-col md:flex-row md:items-center justify-between gap-10 mb-12 pb-10 border-b border-white/5">
                <div className="flex items-center gap-8">
                  ...
                  <div>
                    ...
                    <p className="text-slate-500 text-xs font-black uppercase tracking-[0.3em] mt-2">{s.code} • {s.credits} Credits • {s.lessonsCount} Modules</p>
                  </div>
                </div>
                {canEdit && (
                ...
                <div className="space-y-8">
                  ...
                  <div className="space-y-5">
                    {s.lessonNames?.map((name, i) => {
                      const isEditingThis = editingLesson?.subjectId === s.id && editingLesson?.index === i;
                      const pendingFile = pendingFiles[s.id]?.[i];
                      const currentFile = s.materials?.[i];
                      ...
                      return (
                        <div key={i} className="flex flex-col md:flex-row items-center justify-between p-8 bg-slate-950/50 rounded-[3rem] border border-white/5 group/lesson hover:bg-slate-950 transition-all shadow-inner gap-8">
                          <div className="flex items-center gap-10 min-w-0 flex-1">
                            <div className={`w-16 h-16 rounded-[1.5rem] flex items-center justify-center font-black text-xl shrink-0 shadow-2xl transition-all ${currentFile ? 'bg-emerald-600 text-white' : pendingFile ? 'bg-amber-600 text-white animate-pulse' : 'bg-slate-900 text-slate-500 group-hover/lesson:text-white'}`}>{i + 1}</div>
                              ...
                              {isEditingThis ? (
                                <div className="flex items-center gap-4 animate-in slide-in-from-left-2">
                                  <input
                                    ...
                                    className="bg-black/40 border border-primary/40 rounded-xl px-4 py-2 text-lg text-white font-black outline-none focus:ring-2 focus:ring-primary/20 w-full"
                                    autoFocus
                                  />
                                  <button
                                    onClick={handleUpdateLessonName}
                                    className="px-5 py-3 bg-emerald-600 text-white rounded-xl shadow-lg active:scale-90 transition-all flex items-center gap-2"
                                  >
                                  ...
                                  <button
                                    ...
                                    className="p-3 bg-slate-800 text-slate-400 rounded-xl active:scale-90 transition-all"
                                  >
                                ...
                                <div className="flex items-center gap-4 group/title">
                                  ...
                                  {canEdit && (
                                    <button
                                      onClick={() => setEditingLesson({ subjectId: s.id, index: i, name })}
                                      className="p-2.5 bg-slate-900 border border-white/5 text-slate-500 hover:text-primary hover:border-primary/20 rounded-xl transition-all shadow-md flex items-center gap-2"
                                    >
                                      <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z"></path></svg>
                                      <span className="text-[8px] font-black uppercase tracking-widest">Edit Name</span>
                                    </button>
                                  )}
                                </div>
                              )}
                              <div
                                className={`flex items-center gap-3 mt-2 ${hasFile ? 'cursor-pointer' : ''}`}
                                onClick={() => hasFile && handleOpenAsset(activeFile)}
                              >
                                <div className={`w-2 h-2 rounded-full ${currentFile ? 'bg-emerald-500 animate-pulse' : pendingFile ? 'bg-amber-500 animate-pulse' : 'bg-slate-700'}`}></div>
                                <p className={`text-[11px] font-mono uppercase tracking-widest truncate max-w-[400px] ${hasFile ? 'text-emerald-400 font-bold hover:underline' : 'text-slate-600'}`}>
                                  {pendingFile ? `${pendingFile} (Awaiting Save)` : currentFile || 'Registry Asset Missing'}
                                </p>
                              </div>
                            </div>
                          </div>
                          ...
                          <div className="flex items-center gap-4">
                            {currentFile && (
                              <button
                                onClick={() => handleOpenAsset(currentFile)}
                                className="px-6 py-5 rounded-[1.75rem] text-[10px] font-black uppercase tracking-[0.3em] border border-white/10 bg-slate-900 text-slate-300 hover:bg-slate-800 transition-all shadow-xl active:scale-95 flex items-center gap-3"
                              >
                            ...
                            {pendingFile && (
                              <button
                                onClick={() => handleSaveMaterial(s.id, i)}
                                className="px-8 py-5 bg-emerald-600 text-white rounded-[1.75rem] text-[10px] font-black uppercase tracking-[0.3em] shadow-xl shadow-emerald-900/40 active:scale-95 transition-all flex items-center gap-3"
                              >
                            ...
                            {canEdit && (
                              <button
                                ...
                                className={`min-w-[160px] py-5 rounded-[1.75rem] text-[10px] font-black uppercase tracking-[0.3em] border transition-all shadow-2xl active:scale-95 flex items-center justify-center gap-3 ${hasFile ? 'bg-slate-900 border-white/10 text-slate-400 hover:text-white' : 'bg-primary text-white border-primary/20 hover:bg-indigo-500'}`}
                              >
                                <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z"></path></svg>
                                {currentFile ? 'Update' : 'Deploy'}
                              </button>
                            )}
                          </div>
                        </div>
                    ...
                    {s.questionPapers && s.questionPapers.length > 0 ? (
                      s.questionPapers.map((qp, i) => (
                        <div key={i} className="flex items-center justify-between p-6 bg-emerald-500/5 border border-emerald-500/10 rounded-[2.5rem] group/paper hover:border-emerald-500/30 transition-all shadow-xl">
                          <div className="min-w-0 cursor-pointer" onClick={() => handleOpenAsset(qp.file)}>
                            <p className="text-sm font-black text-emerald-400 uppercase tracking-tighter truncate group-hover/paper:underline">{qp.name}</p>
                          ...
                          <div className="flex gap-2">
                            {canEdit && (
                              <>
                                <button
                                  onClick={() => handleEditQP(s.id, qp, i)}
                                  className="w-10 h-10 bg-slate-950 text-slate-400 rounded-xl flex items-center justify-center hover:text-primary transition-all active:scale-90 border border-white/5"
                                >
                            ...
                            <button
                              onClick={() => handleOpenAsset(qp.file)}
                              className="w-10 h-10 bg-emerald-600 text-white rounded-xl flex items-center justify-center hover:bg-emerald-500 transition-all active:scale-90 shadow-lg shadow-emerald-900/40"
                            >
      ...
      {/* Persistence Modal - Subject Registration */}
      {isAddModalOpen && (
      ...
      {isQPModalOpen && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/80 backdrop-blur-xl p-4">
          <div className="bg-[#0b1121] border border-white/10 p-12 md:p-16 rounded-[4rem] max-w-md w-full shadow-[0_0_100px_rgba(0,0,0,0.9)] animate-in zoom-in-95 duration-300">
            <h3 className="text-3xl font-black text-white uppercase tracking-tighter mb-12 leading-tight">Registry Paper<br /><span className="text-emerald-400 lowercase font-normal italic tracking-normal">{newQP.isEditing ? 'modification' : 'synchronizer'}</span></h3>
            <div className="space-y-12">
              <div className="space-y-3">
                <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest ml-1">Target Assessment Unit</label>
                <select
                  disabled={newQP.isEditing}
                  value={newQP.subjectId}
                  onChange={e => setNewQP({ ...newQP, subjectId: e.target.value })}
                  className="w-full bg-slate-950 border border-white/10 rounded-2xl px-8 py-5 text-white font-bold outline-none appearance-none shadow-inner cursor-pointer disabled:opacity-50"
                >
              ...
              <div className="space-y-3">
                ...
                <div className="mt-4">
                  ...
                  >
                    {newQP.file ? 'Replace Archive' : 'Sync Archive File'}
                  </button>
                </div>
              ...
              <div className="flex gap-6 pt-12">
                ...
                >
                  {newQP.isEditing ? 'Authorize Update' : 'Authorize Paper'}
                </button>
              </div>
            </div>
          </div>
