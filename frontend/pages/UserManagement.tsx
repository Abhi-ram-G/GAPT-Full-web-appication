...
import { User, UserRole, UserStatus, AcademicBatch, Course, AccessLevel, Feature } from '../types';
...
const UserManagement: React.FC = () => {
  ...
  const [newStudent, setNewStudent] = useState({
    email: '', bulkEmails: '', bulkNames: '', name: '', departmentId: '', studyYear: '1st Year', regNo: '', batchType: 'UG' as 'UG' | 'PG', batchId: ''
  });

  const [newStaff, setNewStaff] = useState({
    email: '', bulkEmails: '', bulkNames: '', name: '', role: UserRole.STAFF, department: ''
  });
  ...
  const hasAccess = (feature: Feature) => {
    const level = permissions[currentView]?.[feature] || AccessLevel.NO_ACCESS;
    return level !== AccessLevel.NO_ACCESS;
  };

  const availableStudentDepartments = useMemo(() => {
    if (enrollmentCategory !== 'STUDENT' || !newStudent.batchId) return [];
    const matchingBatch = batches.find(b => b.id === newStudent.batchId);
    if (!matchingBatch) return [];
    return matchingBatch.departmentIds.map(id => courses.find(c => c.id === id)).filter(Boolean) as Course[];
  }, [newStudent.batchId, batches, courses, enrollmentCategory]);
  ...
  const handleBatchChange = (batchId: string) => {
    ...
    const diff = currentYear - selectedBatch.startYear;
    let newYear = '1st Year';

    if (diff > 0) {
      const academicYear = currentMonth >= 5 ? diff + 1 : diff; // June is month 5
    ...
    setNewStudent(prev => ({
      ...
      batchId,
      studyYear: newYear,
      batchType: selectedBatch.batchType as 'UG' | 'PG',
      departmentId: ''
    }));
  };
  ...
  const handleAddMember = async (e: React.FormEvent) => {
    ...
    try {
      if (enrollmentCategory === 'STUDENT') {
        ...
        if (enrollmentMode === 'SINGLE') {
          ...
          await ApiService.addUser({
            id: `usr_${Date.now().toString(36)}_${Math.random().toString(36).substring(2)}`, username: email, email: email, name: newStudent.name.toUpperCase().trim(),
          ...
          const names = newStudent.bulkNames.split(',').map(n => n.trim().toUpperCase()).filter(n => n.length > 0);

          for (let i = 0; i < emails.length; i++) {
            const email = emails[i];
            const nameToUse = names[i] || email.split('@')[0].replace('.', ' ').toUpperCase();
            await ApiService.addUser({
              id: `usr_${Date.now().toString(36)}_${Math.random().toString(36).substring(2)}`, username: email, email: email, name: nameToUse,
              role: UserRole.STUDENT, status: UserStatus.APPROVED, createdAt: new Date().toISOString(), password: 'stdbitsathy',
        ...
        if (enrollmentMode === 'SINGLE') {
          ...
          await ApiService.addUser({
            id: `usr_${Date.now().toString(36)}_${Math.random().toString(36).substring(2)}`, username: email, email: email, name: newStaff.name.toUpperCase().trim(),
          ...
          const names = newStaff.bulkNames.split(',').map(n => n.trim().toUpperCase()).filter(n => n.length > 0);
      ...
      await fetchData();
      setNewStudent({ email: '', bulkEmails: '', bulkNames: '', name: '', departmentId: '', studyYear: '1st Year', regNo: '', batchType: 'UG', batchId: '' });
      setNewStaff({ email: '', bulkEmails: '', bulkNames: '', name: '', role: UserRole.STAFF, department: '' });
    } catch (err: any) {
      console.error(err);
      alert(err.message || 'Error occurred while saving users!');
    }
  };
  ...
  return (
    <DashboardLayout title="Member Directory" resultCount={filteredUsers.length}>
      <div className="max-w-6xl mx-auto py-2">
        <div className="flex items-center justify-start md:justify-center gap-2 mb-8 overflow-x-auto no-scrollbar">
          ...
          {currentLevel !== AccessLevel.VIEW_ALL && currentLevel !== AccessLevel.NO_ACCESS && (
            <>
              ...
              <button onClick={() => navigate('/admin/create-mail')} className="px-10 py-2.5 rounded-full font-bold text-xs border transition-all bg-surface-component text-text-muted border-border-subtle hover:text-white hover:border-text-muted">Create Identity</button>
            </>
          )}
        </div>
          ...
          {viewMode === 'LIST' && (
            <div className="animate-in fade-in duration-300">
              <div className="flex flex-col gap-6 mb-10">
                <div className="flex flex-col md:flex-row items-center justify-center gap-4 relative">
                  ...
                  {selectedUserIds.length > 0 && (
                    <div className="flex items-center gap-3 animate-in slide-in-from-right-4 duration-300 md:absolute md:right-0">
                      <span className="text-[10px] font-black text-text-muted uppercase tracking-widest mr-2">{selectedUserIds.length} Selected</span>
                      {hasAccess(Feature.EMAIL) && (
                        <button
                          onClick={() => {
                            const emails = users.filter(u => selectedUserIds.includes(u.id)).map(u => u.email).join(',');
                            navigate(`/email?compose=true&to=${emails}`);
                          }}
                          className="flex items-center gap-2 px-6 py-2.5 bg-indigo-500 text-white rounded-xl text-[10px] font-black uppercase tracking-widest shadow-xl shadow-indigo-500/20 hover:scale-105 transition-all"
                        >
                ...
                <div className="space-y-4">
                  <div className="max-w-2xl mx-auto relative group">
                    <input
                      ...
                      className="w-full bg-surface-component border border-border-subtle rounded-3xl px-12 pr-24 py-4 text-sm text-text-primary outline-none focus:border-primary/50 transition-all shadow-inner font-bold placeholder:text-slate-600"
                    />
                    ...
                    {/* Live count badge */}
                    <span className="absolute right-4 top-1/2 -translate-y-1/2 text-[10px] font-black text-primary bg-primary/10 border border-primary/20 px-2.5 py-1 rounded-full tabular-nums pointer-events-none">
              ...
              <div className="bg-surface-component border border-border-subtle rounded-3xl overflow-hidden shadow-2xl overflow-x-auto custom-scrollbar">
                <table className="w-full text-left min-w-[900px]" aria-label={`${activeTab.replace(/_/g, ' ')}s list`}>
                    ...
                    <tr>
                      <th className="px-8 py-5 w-16">
                        <input
                          ...
                          className="w-5 h-5 rounded border-slate-700 bg-slate-900 text-primary focus:ring-primary/40 cursor-pointer"
                        />
                      </th>
                      ...
                      <tr key={u.id} className={`hover:bg-black/5 transition-all cursor-pointer group ${selectedUserIds.includes(u.id) ? 'bg-primary/5' : ''}`} onClick={() => toggleSelectUser(u.id)}>
                        <td className="px-8 py-6 w-16" onClick={(e) => e.stopPropagation()}>
                          <input
                            ...
                            className="w-5 h-5 rounded border-slate-700 bg-slate-900 text-primary focus:ring-primary/40 cursor-pointer disabled:opacity-0"
                          />
                        </td>
                          ...
                          <p
                            onClick={(e) => {
                              if (canEditCurrentTab) {
                                e.stopPropagation();
                                setEditingUser(u);
                                setNewPassVal(u.password || '');
                              }
                            }}
                            className={`text-[11px] font-mono font-black text-primary bg-primary/5 inline-block px-3 py-1 rounded-lg border border-primary/20 ${canEditCurrentTab ? 'cursor-pointer hover:bg-primary/10 hover:border-primary/40 transition-all' : ''}`}
                            title={canEditCurrentTab ? "Click to edit password" : ""}
                          >
                        ...
                        <td className="px-8 py-6 text-right" onClick={(e) => e.stopPropagation()}>
                          <div className="flex items-center justify-end gap-2">
                            {hasAccess(Feature.EMAIL) && (
                              <button
                                onClick={() => navigate(`/email?compose=true&to=${u.email}`)}
                                className="p-2.5 bg-indigo-500/10 text-indigo-500 hover:bg-indigo-500 hover:text-white rounded-xl transition-all border border-indigo-500/20"
                                title="Send Email"
                              >
                                <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z"></path></svg>
                              </button>
                            ...
                            {canEditCurrentTab && u.email !== ADMIN_EMAIL && (
                              <button
                                ...
                                className="p-2.5 bg-red-500/10 text-red-500 hover:bg-red-500 hover:text-white rounded-xl transition-all active:scale-90 border border-red-500/20 shadow-lg shadow-red-900/10 focus-visible:ring-2 focus-visible:ring-red-500 outline-none"
                              >
                                <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1-1v3M4 7h16"></path></svg>
                              </button>
                            )}
                          </div>
                        </td>
        ...
        {
          isAddModalOpen && (
            <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/80 backdrop-blur-md p-4 overflow-y-auto" role="dialog" aria-modal="true" aria-labelledby="modal-title">
              <div className="bg-[#0b1121] border border-white/5 p-8 md:p-12 rounded-[3.5rem] max-w-xl w-full shadow-2xl animate-in zoom-in-95 duration-300 relative my-10">
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-10">
                  <h3 id="modal-title" className="text-2xl font-black text-white uppercase tracking-tight">REGISTER {enrollmentCategory}</h3>
                  <div className="flex bg-black/40 p-1 rounded-2xl border border-white/5 shadow-inner" role="radiogroup" aria-label="Enrollment mode">
                    <button
                      role="radio"
                      aria-checked={enrollmentMode === 'SINGLE'}
                      onClick={() => setEnrollmentMode('SINGLE')}
                      className={`px-5 py-2 rounded-xl text-[9px] font-black uppercase tracking-widest transition-all ${enrollmentMode === 'SINGLE' ? 'bg-primary text-white shadow-xl shadow-primary/20' : 'text-slate-500 hover:text-slate-300'}`}
                    >
                      Single
                    </button>
                    <button
                      role="radio"
                      aria-checked={enrollmentMode === 'BULK'}
                      onClick={() => setEnrollmentMode('BULK')}
                      className={`px-5 py-2 rounded-xl text-[9px] font-black uppercase tracking-widest transition-all ${enrollmentMode === 'BULK' ? 'bg-primary text-white shadow-xl shadow-primary/20' : 'text-slate-500 hover:text-slate-300'}`}
                    >
                      Bulk
                    </button>
                  </div>
                ...
                <form onSubmit={handleAddMember} className="space-y-6">
                  {enrollmentCategory === 'STUDENT' ? (
                      ...
                      <div className="space-y-4">
                        <>
                          <div className="space-y-2">
                            <label htmlFor="batch-id" className="text-[10px] font-black text-slate-500 uppercase tracking-widest ml-1">Registry Cohort</label>
                            <select id="batch-id" required value={newStudent.batchId} onChange={e => handleBatchChange(e.target.value)} className="w-full bg-black/40 border border-white/5 rounded-2xl px-6 py-4 text-sm text-white outline-none appearance-none cursor-pointer font-bold shadow-inner">
                          ...
                          <div className="grid grid-cols-2 gap-4">
                            <div className="space-y-1">
                              <label htmlFor="study-year" className="text-[10px] font-black text-slate-500 uppercase tracking-widest ml-1">Study Year</label>
                              <input id="study-year" readOnly value={newStudent.studyYear} className="w-full bg-black/60 border border-white/5 rounded-2xl px-6 py-4 text-sm text-white outline-none font-bold shadow-inner text-slate-400 cursor-not-allowed" />
                            </div>
                            <div className="space-y-1">
                              <label htmlFor="batch-type" className="text-[10px] font-black text-slate-500 uppercase tracking-widest ml-1">Academic Division</label>
                              <input id="batch-type" readOnly value={`${newStudent.batchType} Division`} className="w-full bg-black/60 border border-white/5 rounded-2xl px-6 py-4 text-sm text-white outline-none font-bold shadow-inner text-slate-400 cursor-not-allowed" />
                            </div>
                          </div>
                        </>

                        <div className="space-y-2">
                          <label htmlFor="student-dept" className="text-[10px] font-black text-slate-500 uppercase tracking-widest ml-1">Department</label>
                          <select id="student-dept" required value={newStudent.departmentId} onChange={e => setNewStudent({ ...newStudent, departmentId: e.target.value })} className="w-full bg-black/40 border border-white/5 rounded-2xl px-6 py-4 text-sm text-white outline-none appearance-none cursor-pointer font-bold shadow-inner">
                        ...
                        <div className="animate-in slide-in-from-top-4 duration-300 space-y-4">
                          {enrollmentMode === 'SINGLE' ? (
                            <>
                              <div className="space-y-1">
                                <label htmlFor="student-name" className="text-[10px] font-black text-slate-500 uppercase tracking-widest ml-1">Full Name</label>
                                <input id="student-name" type="text" placeholder="e.g. JOHN DOE" required value={newStudent.name} onChange={e => setNewStudent({ ...newStudent, name: e.target.value })} className="w-full bg-black/40 border border-white/5 rounded-2xl px-6 py-4 text-sm text-white outline-none focus:ring-1 focus:ring-primary/40 transition-all font-bold" />
                              </div>
                              <div className="space-y-1">
                                <label htmlFor="student-email" className="text-[10px] font-black text-slate-500 uppercase tracking-widest ml-1">Email Address</label>
                                <input id="student-email" type="email" placeholder="e.g. john@bitsathy.ac.in" required value={newStudent.email} onChange={e => setNewStudent({ ...newStudent, email: e.target.value })} className="w-full bg-black/40 border border-white/5 rounded-2xl px-6 py-4 text-sm text-white outline-none font-mono focus:ring-1 focus:ring-primary/40 transition-all" />
                              </div>
                            </>
                          ) : (
                            <div className="space-y-2">
                              <label htmlFor="bulk-emails" className="text-[10px] font-black text-slate-500 uppercase tracking-widest ml-1">Mail ID List (Separated by Comma)</label>
                              <textarea id="bulk-emails" placeholder="name1@bitsathy.ac.in, name2@bitsathy.ac.in..." required rows={5} value={newStudent.bulkEmails} onChange={e => setNewStudent({ ...newStudent, bulkEmails: e.target.value })} className="w-full bg-black/40 border border-white/5 rounded-3xl px-6 py-5 text-sm text-white outline-none font-mono resize-none custom-scrollbar shadow-inner" />
                            </div>
                          )}
                        </div>
                      </div>
                    ...
                    <>
                      <div className="space-y-4">
                        <div className="space-y-2">
                          <label htmlFor="staff-dept" className="text-[10px] font-black text-slate-500 uppercase tracking-widest ml-1">Department</label>
                          <select id="staff-dept" required value={newStaff.department} onChange={e => setNewStaff({ ...newStaff, department: e.target.value })} className="w-full bg-black/40 border border-white/5 rounded-2xl px-6 py-4 text-sm text-white outline-none appearance-none cursor-pointer font-bold shadow-inner">
                        ...
                        <div className="animate-in slide-in-from-top-4 duration-300 space-y-4">
                          {enrollmentCategory !== 'ADMIN' && (
                            <div className="space-y-2">
                              <label htmlFor="staff-role" className="text-[10px] font-black text-slate-500 uppercase tracking-widest ml-1">Member Role</label>
                              <select id="staff-role" required value={newStaff.role} onChange={e => setNewStaff({ ...newStaff, role: e.target.value as UserRole })} className="w-full bg-black/40 border border-white/5 rounded-2xl px-6 py-4 text-sm text-white outline-none appearance-none cursor-pointer font-bold shadow-inner">
                          ...
                          {enrollmentMode === 'SINGLE' ? (
                            <>
                              <div className="space-y-1">
                                <label htmlFor="staff-name" className="text-[10px] font-black text-slate-500 uppercase tracking-widest ml-1">Full Name</label>
                                <input id="staff-name" type="text" placeholder="e.g. JANE SMITH" required value={newStaff.name} onChange={e => setNewStaff({ ...newStaff, name: e.target.value })} className="w-full bg-black/40 border border-white/5 rounded-2xl px-6 py-4 text-sm text-white outline-none focus:ring-1 focus:ring-primary/40 transition-all font-bold" />
                              </div>
                              <div className="space-y-1">
                                <label htmlFor="staff-email" className="text-[10px] font-black text-slate-500 uppercase tracking-widest ml-1">Email Address</label>
                                <input id="staff-email" type="email" placeholder="e.g. jane@bitsathy.ac.in" required value={newStaff.email} onChange={e => setNewStaff({ ...newStaff, email: e.target.value })} className="w-full bg-black/40 border border-white/5 rounded-2xl px-6 py-4 text-sm text-white outline-none font-mono focus:ring-1 focus:ring-primary/40 transition-all" />
                              </div>
                            </>
                          ) : (
                            <div className="space-y-2">
                              <label htmlFor="staff-bulk-emails" className="text-[10px] font-black text-slate-500 uppercase tracking-widest ml-1">Mail ID List (Separated by Comma)</label>
                              <textarea id="staff-bulk-emails" placeholder="prof1@bitsathy.ac.in, prof2@bitsathy.ac.in..." required rows={5} value={newStaff.bulkEmails} onChange={e => setNewStaff({ ...newStaff, bulkEmails: e.target.value })} className="w-full bg-black/40 border border-white/5 rounded-3xl px-6 py-5 text-sm text-white outline-none font-mono resize-none custom-scrollbar shadow-inner" />
                            </div>
                          )}
                        </div>
                      </div>
        ...
        {
          editingUser && (
            <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/80 backdrop-blur-md p-4" role="dialog" aria-modal="true" aria-labelledby="edit-modal-title">
              <div className="bg-[#0b1121] border border-white/5 p-8 md:p-12 rounded-[3.5rem] max-w-md w-full shadow-2xl animate-in zoom-in-95 duration-300 relative">
                <h3 id="edit-modal-title" className="text-2xl font-black text-white uppercase tracking-tight mb-8">Update Password</h3>
                <div className="space-y-6">
                  <div className="space-y-2">
                    <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest ml-1">New Password for {editingUser.name}</label>
                    <input
                      type="text"
                      value={newPassVal}
                      onChange={e => setNewPassVal(e.target.value)}
                      className="w-full bg-black/40 border border-white/5 rounded-2xl px-6 py-4 text-sm text-white outline-none focus:border-primary/50 transition-all font-bold shadow-inner"
                  ...
                  <div className="flex gap-4 pt-4">
                    <button
                      ...
                      className="flex-1 py-4 bg-surface-component text-text-muted rounded-[2rem] text-[10px] font-black uppercase tracking-[0.3em] hover:text-white transition-colors"
                    >
                      Cancel
                    </button>
                    <button
                      onClick={async () => {
                        ...
                        await ApiService.updateUser(editingUser.id, { password: newPassVal.trim() });
                        setEditingUser(null);
                        setNewPassVal('');
                        fetchData();
                      }}
                      className="flex-1 py-4 bg-primary hover:bg-indigo-500 text-white rounded-[2rem] text-[10px] font-black uppercase tracking-[0.3em] shadow-xl shadow-primary/20 active:scale-95 transition-all"
                    >
                      Save Changes
                    </button>
                  </div>
