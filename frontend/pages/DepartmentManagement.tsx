...
const DepartmentManagement: React.FC = () => {
  ...
  const [newBatch, setNewBatch] = useState({ batchNumber: 24, batchType: 'UG' as 'UG' | 'PG', selectedDepts: [] as string[] });
  ...
  const persist = async (updatedCourses: Course[], updatedBatches: AcademicBatch[]) => {
    try {
      ...
      await loadData(); // Revert to confirmed remote state
      return false;
    }
  };
  ...
  const handleCreateBatch = async (e: React.FormEvent) => {
    e.preventDefault();
    const startYear = 2000 + newBatch.batchNumber;
    const endYear = startYear + (newBatch.batchType === 'UG' ? 4 : 2);

    const batch: AcademicBatch = {
      id: crypto.randomUUID(),
      name: `${startYear} - ${endYear} COHORT`,
      startYear,
      endYear,
      batchType: newBatch.batchType,
      departmentIds: newBatch.selectedDepts
    };
    const success = await persist(courses, [...batches, batch]);
    if (success) {
      setIsAddBatchModalOpen(false);
      setNewBatch({ batchNumber: 24, batchType: 'UG', selectedDepts: [] });
    }
  };
  ...
  const handleCreateDept = async (e: React.FormEvent) => {
    ...
    const success = await persist(updatedCourses, batches);
    if (success) {
  ...
  const deleteCourse = async (id: string, e: React.MouseEvent) => {
    ...
    if (confirm('Delete this department from the master registry? This will also unregister it from all active institutional batches.')) {
      try {
        ...
        await persist(updatedCourses, updatedBatches); // Ensure frontend state matches
      } catch (error: any) {
                ...
                <div key={batch.id} onClick={() => navigate(`/admin/departments/batch/${batch.id}`)} className="bg-surface-component border border-border-subtle rounded-[2.5rem] p-8 cursor-pointer flex items-center justify-between group transition-all duration-300 hover:border-indigo-500/50 shadow-lg">
                  <div className="flex items-center gap-8">
                    <div className="w-16 h-16 rounded-full bg-primary/10 border border-primary/20 flex items-center justify-center text-primary font-black text-2xl">
                      {batch.endYear.toString().slice(-2)}
                    </div>
                    <div>
            ...
            <form onSubmit={handleCreateBatch} className="space-y-8">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div className="space-y-2">
                  <label className="text-[10px] font-black text-text-muted uppercase tracking-widest ml-1">Batch Number</label>
                  <select required value={newBatch.batchNumber} onChange={e => setNewBatch({ ...newBatch, batchNumber: parseInt(e.target.value) || 24 })} className="w-full bg-surface-deep border border-border-subtle rounded-2xl px-6 py-4 text-sm text-text-primary font-bold outline-none focus:ring-1 focus:ring-indigo-500 transition-all font-mono">
                    {[...Array(20)].map((_, i) => {
                      const startYear = 2000 + 15 + i;
                      const endYear = startYear + (newBatch.batchType === 'UG' ? 4 : 2);
                      return (
                        <option key={15 + i} value={15 + i}>
                          {endYear.toString().slice(-2)} ({startYear} - {endYear})
                        </option>
                      );
                    })}
                  </select>
                  <p className="text-[10px] font-black text-indigo-400 uppercase tracking-[0.2em] mt-2 ml-2">
                    Calculated Cohort: {2000 + newBatch.batchNumber} — {2000 + newBatch.batchNumber + (newBatch.batchType === 'UG' ? 4 : 2)}
                  </p>
                </div>
                <div className="space-y-2">
