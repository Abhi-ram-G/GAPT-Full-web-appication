  ...
  return (
    <DashboardLayout title="Institutional Command Center">
      ...
      <div className="space-y-8">
        <div className="bg-surface-component border border-border-subtle rounded-3xl md:rounded-[3rem] overflow-hidden shadow-2xl">
          <div className="p-6 md:p-8 border-b border-border-subtle bg-black/5 dark:bg-slate-800/20 flex flex-col lg:flex-row lg:items-center justify-between gap-6">
          ...
          <div className="overflow-x-auto custom-scrollbar">
            <table className="w-full text-left border-collapse min-w-[700px]" aria-label="Department-wise staff attendance breakdown">
...
const StatCard: React.FC<{ title: string; value: number; icon: string; color: string }> = ({ title, value, color }) => (
  <div className="bg-surface-component border border-border-subtle rounded-3xl md:rounded-[2.5rem] p-6 md:p-6 transition-all hover:scale-[1.02] hover:shadow-2xl shadow-slate-950/50 group relative overflow-hidden flex flex-col items-center md:items-start text-center md:text-left">
    ...
    <div className="flex items-center justify-between w-full mb-4 md:mb-4">
      <span className="text-text-muted text-[10px] md:text-[9px] font-bold uppercase tracking-widest">{title}</span>
      <div className={`w-8 h-8 md:w-8 md:h-8 rounded-lg bg-surface-elevated flex items-center justify-center border border-border-subtle ${color}`}>
        <div className="w-1.5 h-1.5 md:w-1.5 md:h-1.5 rounded-full bg-current"></div>
      </div>
    </div>
    <div className="text-3xl md:text-3xl font-black text-text-primary tracking-tighter">{value}</div>
  </div>
);
